...
## Part 2
```{r ggplot, echo=FALSE, message=FALSE, warning=FALSE, out.width="25%"}
library(gapminder)
library(tidyverse)
library(dplyr)
library(here)
...
#Instead of colors being used to make trends in features clear it's highlighting years, which are already obvious based on the x-axis. Colors are inconsistent and low contrast.Overall trend line doesn't communicate anything meaningful. Title is not wrapped to the plot, y-label is missing, x-label is uninformative, and x-axis breaks are inconsistent.I also just think the dark theme is (in general) ugly and not that readable. 
